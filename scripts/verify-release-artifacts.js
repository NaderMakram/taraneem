...
function verifyListedFiles(directory, metadata) {
  for (const file of metadata.files) {
    ...
    invariant(
      ...
      "Update URL must be a file name: " + file.url
    );
    invariant(
      typeof file.sha512 === "string" && file.sha512.length > 0,
      "Missing SHA-512 for " + file.url
    );

    const artifactPath = path.join(directory, file.url);
    invariant(
      fs.existsSync(artifactPath),
      "Metadata references missing artifact " + file.url
    );
    ...
    invariant(
      file.sha512 === sha512(artifactPath),
      "Incorrect SHA-512 for " + file.url
    );
  }
}
...
function verifyReleaseArtifacts({ platform, version, directory }) {
  ...
  invariant(
    ...
    "Release directory does not exist: " + resolvedDirectory
  );
  ...
  if (platform === "windows") {
    ...
    invariant(
      fs.existsSync(path.join(resolvedDirectory, installerName)),
    ...
    invariant(
      ...
      "latest.yml changed the legacy Windows installer path"
    );
    invariant(
  ...
  if (platform === "macos") {
    const installerName =
      "Taraneem-macOS-" + version + "-universal.dmg";
    const installerPath = path.join(resolvedDirectory, installerName);

    invariant(fs.existsSync(installerPath), "Missing " + installerName);
    invariant(
      fs.statSync(installerPath).size > 0,
      installerName + " must not be empty"
    );

    return { installer: installerName, updateMode: "manual" };
  }
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    invariant(
      ...
      "Invalid argument near " + (key || "the end")
    );
...
if (require.main === module) {
  try {
    ...
    console.log(
      "Verified " + options.platform + " release artifacts: " + JSON.stringify(result)
    );
  } catch (error) {
